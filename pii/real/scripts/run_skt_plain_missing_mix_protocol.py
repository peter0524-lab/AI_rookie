...
TRAIN_SPECS = [
    ("mix_syn025", "Mix-0.25", "0.25"),
    ("mix_syn050", "Mix-0.50", "0.50"),
    ("mix_syn100", "Mix-1.00", "1.00"),
]
...
def eval_one(train_set, train_display, ratio, test_set, scope):
    ...
    cmd = [
        ...
        "--no_cache",
        "--tag", tag,
        "--batch_size", os.environ.get("SKT_PLAIN_EVAL_BSZ", "64"),
        "--max_length", os.environ.get("SKT_PLAIN_MAX_LEN", "256"),
    ]
    if scope == "common18":
...
def main():
    log(f"[protocol-start] missing SKT-only mix ratios train_sets={[x[0] for x in TRAIN_SPECS]} tests={TEST_SETS} scopes={LABEL_SCOPES}")
    failures = []
    for train_set, train_display, ratio in TRAIN_SPECS:
        if not train_one(train_set, train_display, ratio):
            failures.append(f"train:{train_set}")
            continue
        for test_set in TEST_SETS:
            for scope in LABEL_SCOPES:
                if not eval_one(train_set, train_display, ratio, test_set, scope):
                    failures.append(f"eval:{train_set}:{test_set}:{scope}")
    if failures:
        log(f"[protocol-complete-with-failures] n={len(failures)} failures={failures}")
        raise SystemExit(1)
    log("[protocol-complete] all requested missing SKT-only mix-ratio metrics are ready")
