...
# Default backend set includes Meta Llama, so two datasets run 14 jobs total.
#
# DATASET_SPECS format:
#   "name1:train_json:test_json name2:train_json:test_json"
#
# Recommended server layout:
#   data_32k_final/full_train.json
#   data_32k_final/full_test.json
#   data_32k_data/full_train.json
#   data_32k_data/full_test.json
set -euo pipefail
cd "$(dirname "$0")/.."
...
MODEL_IDS_TEXT="${MODEL_IDS_TEXT:-skt/A.X-4.0-Light LGAI-EXAONE/EXAONE-Deep-7.8B NCSOFT/Llama-VARCO-8B-Instruct upstage/SOLAR-10.7B-Instruct-v1.0 Qwen/Qwen3-8B mistralai/Mistral-7B-Instruct-v0.3 meta-llama/Llama-3.1-8B-Instruct}"
...
사용 예:
  DATASET_SPECS="final:data_32k_final/full_train.json:data_32k_final/full_test.json data:data_32k_data/full_train.json:data_32k_data/full_test.json" \
  bash scripts/run_two_datasets_original_regularized_with_llama.sh
EOF
        exit 2
    fi
fi
...
echo "================================================================"
echo "[two-datasets] original full-attention regularized sweep with Llama"
...
echo "전체 2-dataset x 7-backend sweep 완료"
echo "================================================================"
