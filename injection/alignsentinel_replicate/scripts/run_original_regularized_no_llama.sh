#!/usr/bin/env bash
# Original full-attention Enc-first/Avg-first regularized sweep, excluding gated Meta Llama.
# Use a separate RUN_TAG to keep old 16k results and new 32k results apart.
set -euo pipefail
cd "$(dirname "$0")/.."

RUN_TAG="${RUN_TAG:-32k_$(date +%Y%m%d_%H%M%S)}"
...
export MODEL_IDS_TEXT="${MODEL_IDS_TEXT:-skt/A.X-4.0-Light LGAI-EXAONE/EXAONE-Deep-7.8B NCSOFT/Llama-VARCO-8B-Instruct upstage/SOLAR-10.7B-Instruct-v1.0 Qwen/Qwen3-8B mistralai/Mistral-7B-Instruct-v0.3}"
