#!/usr/bin/env bash
# Run the original full-attention regularized detector on two dataset bundles
# with EXAONE backends only.
#
# Default jobs:
#   1) data_32k_final x LGAI-EXAONE/EXAONE-3.5-2.4B-Instruct
#   2) data_32k_final x LGAI-EXAONE/EXAONE-3.5-7.8B-Instruct
#   3) data_32k_final x LGAI-EXAONE/EXAONE-4.0-1.2B
#   4) data_32k_data  x LGAI-EXAONE/EXAONE-3.5-2.4B-Instruct
#   5) data_32k_data  x LGAI-EXAONE/EXAONE-3.5-7.8B-Instruct
#   6) data_32k_data  x LGAI-EXAONE/EXAONE-4.0-1.2B
set -euo pipefail
cd "$(dirname "$0")/.."

RUN_TAG_PREFIX="${RUN_TAG_PREFIX:-32k_exaone}"
SKIP_DONE="${SKIP_DONE:-1}"
CONTINUE_ON_ERROR="${CONTINUE_ON_ERROR:-1}"
CONTINUE_DATASETS_ON_ERROR="${CONTINUE_DATASETS_ON_ERROR:-1}"
CLEAN_FEATURES_AFTER="${CLEAN_FEATURES_AFTER:-1}"

MODEL_IDS_TEXT="${MODEL_IDS_TEXT:-LGAI-EXAONE/EXAONE-3.5-2.4B-Instruct LGAI-EXAONE/EXAONE-3.5-7.8B-Instruct LGAI-EXAONE/EXAONE-4.0-1.2B}"
export MODEL_IDS_TEXT SKIP_DONE CONTINUE_ON_ERROR CLEAN_FEATURES_AFTER

if [[ -z "${DATASET_SPECS:-}" ]]; then
    if [[ -f data_32k_final/full_train.json && -f data_32k_final/full_test.json && -f data_32k_data/full_train.json && -f data_32k_data/full_test.json ]]; then
        DATASET_SPECS="final:data_32k_final/full_train.json:data_32k_final/full_test.json data:data_32k_data/full_train.json:data_32k_data/full_test.json"
    elif [[ -f data_32k_1/full_train.json && -f data_32k_1/full_test.json && -f data_32k_2/full_train.json && -f data_32k_2/full_test.json ]]; then
...
사용 예:
  DATASET_SPECS="final:data_32k_final/full_train.json:data_32k_final/full_test.json data:data_32k_data/full_train.json:data_32k_data/full_test.json" \
  bash scripts/run_two_datasets_exaone_small_regularized.sh
EOF
        exit 2
    fi
fi
...
echo "================================================================"
echo "[two-datasets] original full-attention regularized sweep, EXAONE only"
...
echo "[two-datasets] CLEAN_FEATURES_AFTER=${CLEAN_FEATURES_AFTER}"
echo "[two-datasets] models=$(wc -w <<<"${MODEL_IDS_TEXT}")"
printf ' - %s\n' ${MODEL_IDS_TEXT}
...
for SPEC in ${DATASET_SPECS}; do
    ...
    bash scripts/run_original_regularized_with_llama.sh
    STATUS=$?
    set -e
...
echo "전체 2-dataset x 3-EXAONE sweep 완료"
echo "================================================================"
